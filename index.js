...
async function handleUpdate(update, env) {
  try {
    ...
    if (userIsAdmin && (text === "تگ" || text === "تگ کل" || text.startsWith("تگ کل ") || text.startsWith("تگ "))) {
      let customTagMsg = "";
      if (text.startsWith("تگ کل ")) {
        customTagMsg = text.replace("تگ کل", "").trim();
      } else if (text.startsWith("تگ ") && text !== "تگ") {
        customTagMsg = text.replace("تگ", "").trim();
      }
